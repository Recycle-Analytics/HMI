...
class _LCD_i80(Module, AutoCSR):
    def __init__(self,clk):
        ...
        TenNanoSecond   =   int(10e-9*clk)
        delay           =   5*TenNanoSecond
        delay2          =   15*TenNanoSecond
        ...
        fsm.act("MANAGER",
            ...
            If(count2<delay2,NextState("MANAGER")),
            If(count2>=delay2,
                If(manage==0x0,
        ...
        fsm.act("WAIT_START",
            ...
            If(countwr<delay,NextState("WAIT_START")),
            If(countwr>=delay,
                NextState("START"),
            )
        )
        fsm.act("MANAGERWR",
            NextValue(countwr,countwr+1),
            If(countwr<delay2,NextState("MANAGERWR")),
            If(countwr>=delay2,
                NextState("WRSTROBE"),
            )
        )
